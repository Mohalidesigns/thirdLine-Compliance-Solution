...
export default function OwnerModal({ open, onClose, obligationId, initial = {}, onSaved, onError }) {
  ...
  async function handleSave() {
    setLoading(true);
    try {
      await api.platform.obligations.assignOwner?.(obligationId, {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth maxWidth="xs">
      ...
      <DialogContent dividers>
        <TextField size="small" fullWidth label="Owner ID"
          value={ownerId ?? ''} onChange={e => setOwnerId(e.target.value ? Number(e.target.value) : null)} sx={{ mt: 1 }} />
