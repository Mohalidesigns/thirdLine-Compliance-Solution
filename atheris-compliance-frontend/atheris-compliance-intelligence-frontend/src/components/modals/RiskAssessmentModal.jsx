...
export default function RiskAssessmentModal({ open, onClose, obligationId, initial = {}, onSaved, onError }) {
  ...
  useEffect(() => {
    if (open) {
      setForm({
        tenantRiskRating: initial.inherentRiskRating || initial.riskRating || '',
        riskJustification: initial.riskDescription || '',
        impactRating: initial.inherentImpact || '',
        likelihoodRating: initial.inherentLikelihood || '',
        changeReason: '',
      });
    }
  }, [open, initial]);
  ...
  async function handleSave() {
    setLoading(true);
    try {
      await api.platform.obligations.updateRisk?.(obligationId, {
