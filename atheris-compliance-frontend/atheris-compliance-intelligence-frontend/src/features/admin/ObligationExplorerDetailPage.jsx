...
export default function ObligationExplorerDetailPage() {
  ...
  const { data: selected, isLoading, error } = useQuery({
    ...
    queryFn: ({ signal }) => api.platform.obligations.get(obligationId, signal),
  ...
  async function handleDownloadEvidence(ev) {
    try {
      const { blob, name } = await api.platform.evidence.download(ev.fileId);
  ...
  const openDrawer = (section, singleId = null) => {
    ...
    if (section === 'controls' && singleId) {
      ...
      api.platform.controls.detail(singleId).then(d => setDrawerControlDetail(d)).catch(() => {});
    } else {
      setDrawerControlDetail(null);
    }
  };
  ...
  if (error && !selected) return (
    <Box>
      <IconButton onClick={() => navigate('/admin/obligations')} sx={{ mb: 2 }}><ArrowBack /></IconButton>
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
        <IconButton onClick={() => navigate('/admin/obligations')}><ArrowBack /></IconButton>
        <Box sx={{ flex: 1 }} />
        ...
        <Box sx={{ maxWidth: 900 }}>
          ...
          <Box sx={{ mb: 2 }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap', mb: 1 }}>
              ...
              <Chip size="small" label={selected.hasPoints ? 'With Points' : 'Without Points'}
                color={selected.hasPoints ? 'success' : 'default'} sx={{ height: 22 }} />
            </Box>

            {/* Title */}
            <Typography variant="h6" sx={{ fontWeight: 400, mb: 0.5 }}>
              {selected.title || 'Untitled obligation'}
            </Typography>
            {selected.instrumentTitle && (
              <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>{selected.instrumentTitle}</Typography>
            )}
