...
import {
  Visibility, Download, Edit, UploadFile, ArrowBack, Gavel, Close, Search,
} from '@mui/icons-material';
import FormattedText from './FormattedText.jsx';
import RiskAssessmentModal from '../../atheris-compliance-tenant-frontend/src/components/modals/RiskAssessmentModal';
import OwnerModal from '../../atheris-compliance-tenant-frontend/src/components/modals/OwnerModal';
import LinkControlsModal from '../../atheris-compliance-tenant-frontend/src/components/modals/LinkControlsModal';
import MapReturnModal from '../../atheris-compliance-tenant-frontend/src/components/modals/MapReturnModal';
import GapModal from '../../atheris-compliance-tenant-frontend/src/components/modals/GapModal';
import EvidenceUploadModal from '../../atheris-compliance-tenant-frontend/src/components/modals/EvidenceUploadModal';
...
function controlItemStyle(i, total) {
  return {
    py: 0.75,
    borderBottom: i < Math.min(total, 5) - 1 ? '1px solid' : 'none',
    borderColor: 'divider',
    cursor: 'pointer',
...
const norm = (s) => s ? {
  ...s,
  riskRating: s.riskRating || s.tenantRiskRating,
  inherentRiskRating: s.inherentRiskRating,
  instrumentTitle: s.instrumentTitle || s.sourceTitle || s.instrument?.sourceTitle,
  linkedReturns: s.linkedReturns || s.returns?.map(r => ({ returnName: r.title, ...r })) || [],
  linkedControls: s.linkedControls || [],
} : s;
...
export default function ObligationDetailView({
  ...
  const resolvedQueryKey = queryKey || ['obligation', obligationId];
  const resolvedQueryFn = queryFn || (({ signal }) => {
    if (apiProp?.platform?.obligations?.get) return apiProp.platform.obligations.get(obligationId, signal);
    if (apiProp?.obligations?.obligationDetail) return apiProp.obligations.obligationDetail(obligationId, signal);
    if (apiProp?.obligations?.get) return apiProp.obligations.get(obligationId, signal);
    throw new Error('No queryFn or api provided for obligation detail');
  });
  ...
  const selected = useMemo(() => norm(raw), [raw]);
  ...
  async function handleDownloadEvidence(ev) {
    try {
      let result;
      if (mode === 'intel' && apiProp?.platform?.evidence?.download) {
        result = await apiProp.platform.evidence.download(ev.fileId);
      } else if (apiProp?.evidence?.download) {
        result = await apiProp.evidence.download(ev.fileId);
      } else if (apiProp?.platform?.evidence?.download) {
        result = await apiProp.platform.evidence.download(ev.fileId);
      } else {
        throw new Error('No evidence download api');
      }
  ...
  async function handleViewPdf() {
    ...
    try {
      if (pdfFetcher) {
        const fetched = await pdfFetcher(instrumentId);
        if (fetched instanceof Blob) {
          window.open(URL.createObjectURL(fetched), '_blank');
          return;
        }
        ...
        if (fetched?.blob) {
          window.open(URL.createObjectURL(fetched.blob), '_blank');
          return;
        }
        if (fetched?.url) {
          window.open(fetched.url, '_blank');
          return;
        }
      }
      // fallback to API_BASE fetch with getToken
      const token = getTokenProp ? getTokenProp() : (apiProp?.getToken ? apiProp.getToken() : '');
      const base = API_BASEProp || '';
      let url;
      if (mode === 'intel') {
        url = base ? `${base}/admin/obligations/${instrumentId}/pdf` : `/api/v1/admin/obligations/${instrumentId}/pdf`;
      } else {
        url = base ? `${base}/subscriptions/instruments/${instrumentId}/pdf` : `/api/v1/subscriptions/instruments/${instrumentId}/pdf`;
      }
  ...
  const actionEdit = (modal, label = 'Edit') => (
    <Button size="medium" variant="contained" onClick={() => setActiveModal(modal)}
      ...
      sx={{ width: 180, height: 40, textTransform: 'none', fontWeight: 600, fontSize: 14, whiteSpace: 'nowrap' }}>
      {label}
    </Button>
  );
  ...
  const openDrawer = (section, singleId = null) => {
    ...
    if (section === 'controls' && singleId) {
      ...
      const fetcher = mode === 'intel' && apiProp?.platform?.controls?.detail
        ? apiProp.platform.controls.detail.bind(apiProp.platform.controls)
        : apiProp?.controls?.detail?.bind(apiProp.controls) || null;
      if (fetcher) {
        fetcher(singleId).then(d => setDrawerControlDetail(d)).catch(() => {});
      } else if (apiProp?.platform?.controls?.detail) {
        apiProp.platform.controls.detail(singleId).then(d => setDrawerControlDetail(d)).catch(() => {});
      } else if (apiProp?.controls?.detail) {
        apiProp.controls.detail(singleId).then(d => setDrawerControlDetail(d)).catch(() => {});
      }
    } else {
      setDrawerControlDetail(null);
    }
  ...
  const backPath = onBackPath || (mode === 'intel' ? '/admin/obligations' : '/obligations');

  if (isLoading) return <Box sx={{ display: 'flex', justifyContent: 'center', mt: 8 }}><CircularProgress /></Box>;
  if (error && !selected) return (
    <Box>
      <IconButton onClick={() => navigate(backPath)} sx={{ mb: 2 }}><ArrowBack /></IconButton>
  ...
  return (
    <Box>
      ...
      {selected && (
        <Box sx={{ maxWidth: 900 }}>
          {/* Header — reverted to bd289a7 simple layout */}
          <Box sx={{ mb: 1.5 }}>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1.5 }}>
              {riskChip(selected.riskRating || selected.inherentRiskRating)}
              {selected.areaOfFocus && <Chip size="small" label={selected.areaOfFocus} sx={{ height: 22, borderRadius: '4px' }} />}
              {selected.obligationType && <Chip size="small" label={selected.obligationType} sx={{ height: 22, borderRadius: '4px' }} />}
              <Chip size="small" label={selected.hasPoints ? 'With Points' : 'Without Points'} color={selected.hasPoints ? 'success' : 'default'} sx={{ height: 22, borderRadius: '4px' }} />
            </Box>
            <Typography variant="h6" sx={{ fontWeight: 400, mb: 1.5 }}>{selected.title || 'Untitled obligation'}</Typography>
            <Divider sx={{ mb: 2 }} />
            <Paper variant="outlined" sx={{ p: 3, mb: 2, bgcolor: 'background.paper' }}>
              <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px 32px' }}>
                {[
                  ['Instrument', selected.instrumentTitle || selected.instrument?.sourceTitle],
                  ['Regulator', selected.regulatorAbbreviation || selected.regulatorName],
                  ['Section Reference', selected.sectionReference],
                  ['Act Name', selected.actName],
                  ['Risk Description', selected.riskDescription],
                  ['Control Owner', selected.controlOwner],
                  ['Obligation Type', selected.obligationType],
                  ['Recurring Deadline', selected.recurringDeadlineType],
                ].map(([label, value]) => (
                  <Box key={label}>
          ...
          <Paper variant="outlined" sx={{ p: 3, mb: 2 }}>
            <SectionHeader title={`Linked Controls (${(selected.linkedControls || []).length})`}
              action={
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                {(selected.linkedControls || []).slice(0, 5).map((c, i) => (
                  <Box key={c.controlId || i}
                    ...
                    sx={controlItemStyle(i, Math.min((selected.linkedControls || []).length, 5))}>
          ...
          <Paper variant="outlined" sx={{ p: 3, mb: 2 }}>
            <SectionHeader title={`Return Required (${(selected.linkedReturns || []).length})`}
              action={
                <Box sx={{ display: 'flex', gap: 1 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                {(selected.linkedReturns || []).slice(0, 5).map((r, i) => (
                  <Box key={r.returnId || i}
                    ...
                    sx={controlItemStyle(i, Math.min((selected.linkedReturns || []).length, 5))}>
                    <Typography variant="body2">
          ...
          <Paper variant="outlined" sx={{ p: 3, mb: 2 }}>
            <SectionHeader title={`Regulatory Sanctions (${(selected.sanctions || []).length})`}
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                {(selected.sanctions || []).slice(0, 5).map((s, i) => (
                  <Box key={i}
                    ...
                    sx={controlItemStyle(i, Math.min((selected.sanctions || []).length, 5))}>
                    <Typography variant="body2">
                    ...
                    <Box sx={{ display: 'flex', gap: 1, mt: 0.25, flexWrap: 'wrap' }}>
                      ...
                      {s.actName && (
                        <Typography variant="body2" color="text.secondary">{s.actName}</Typography>
                      )}
                    </Box>
                  </Box>
                ))}
          ...
          <Paper variant="outlined" sx={{ p: 3, mb: 2 }}>
            <SectionHeader title={`Evidence (${(selected.evidence || []).length})`}
              action={
                <Box sx={{ display: 'flex', gap: 1 }}>
          ...
          <Box>
            ...
            {drawerSection === 'controls' && drawerSingleId && drawerControlDetail ? (
              ...
              <Box key={c.controlId || i}
                ...
                <Typography variant="body2" sx={{ fontWeight: 500 }}>{c.name || 'Untitled'}</Typography>
                <Box sx={{ display: 'flex', gap: 0.5, mt: 0.5, flexWrap: 'wrap' }}>
